...
foreach ($Twigs as $i => $value) {
        $assoc_twig[$value['name']] = $value['html'];
}
$loader = new \Twig\Loader\ArrayLoader($assoc_twig);
...
$vd = new Twig_SimpleFunction('dump', function ($mixed) {
  ob_start();
  var_dump($mixed);
...
//if ( !file_exists($TwigTemplatesDir.'/'.$templatename) ) {
  ...
  if ( !isset($assoc_twig[$templatename]) )  {
